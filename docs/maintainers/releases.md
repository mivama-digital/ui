...
`@orevori/ui` uses Changesets for release intent and a manual GitHub Actions workflow for npm publishing.
...
Configure the `@orevori/ui` package on npm with a GitHub Actions trusted publisher using exactly:
...
- npm package: `@orevori/ui@<version>`
- Git tag: `v<version>`
