 ...
 *
 * This script deliberately does not write component source. @mivama/ui carries
...
export function renderProposal(report) {
  const lines = [
    ...
    "## Review boundary",
    "",
    "This is a **draft-only detection proposal**. It intentionally does not overwrite `src/components/ui` or update the baseline pin. `@mivama/ui` uses package-local imports and Mivama theme tokens, so every source change must be reviewed, adapted, and pass the full package verification before the pinned SHA is advanced.",
    "",
    ...
    "- Changes in the @mivama/ui 64-component catalog: " +
